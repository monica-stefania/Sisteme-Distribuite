...
def resolve_question(formatted_message):
    # creare socket TCP
...
def ask_question():
    ...
    dest_port = port_dest.get()

    formatted_message = f"intrebare {TEACHER_PORT} {dest_port} {question_text}"

    # pornire thread separat pentru tratarea intrebarii respective
    # astfel, nu se blocheaza interfata grafica!
    threading.Thread(target=resolve_question, args=(formatted_message,)).start()
...
if __name__ == '__main__':
    ...
    port_dest_label = ttk.Label(content, text="Port destinatie sau ALL:")
    port_dest = ttk.Entry(content, width=20)
    port_dest.insert(0, "1701")
    ...
    port_dest_label.grid(column=3, row=2)
    port_dest.grid(column=4, row=2)
