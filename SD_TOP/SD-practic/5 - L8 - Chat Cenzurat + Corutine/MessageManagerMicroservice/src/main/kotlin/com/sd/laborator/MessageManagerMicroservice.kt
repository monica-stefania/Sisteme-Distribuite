...
class MessageManagerMicroservice {
    ...
    companion object Constants {
        ...
        val CENSOR_HOST = System.getenv("CENSOR_HOST") ?: "localhost"
    ...
    private fun censorMessage(text: String): String
    {
        try{
            val censorSocket = Socket("censor", 1700)

            censorSocket.getOutputStream().write((text + "\n").toByteArray())
            censorSocket.getOutputStream().flush()
        ...
        {
            println("CensorMicroservice nu raspunde.")
            return text
        }
    }
                ...
                while (true) {
                    ...
                    when (messageType) {
                        ...
                        "raspuns" -> {
                            ...
                            val raspunsCenzurat = censorMessage(messageBody)

                            val raspunsFormatat = "raspuns $messageDestination $raspunsCenzurat"
                            respondTo(messageDestination.toInt(), raspunsFormatat)
                        }
                    }
                }
            }
