...
class BiddingProcessorMicroservice {
        ...
        receiveProcessedBidsObservable = Observable.create<String> { emitter ->
            while (true) {
                ...
                if (Message.deserialize(receivedMessage.toByteArray()).body == "final") {
                    ...
                    val biddingStatsMsg = Message.create("BiddingProcessor", "count")
                    numberOfMessagesSocket.getOutputStream().write(biddingStatsMsg.serialize())
                    messageProcessorConnection.close()
    ...
    private fun decideAuctionWinner() {
        ...
        try {
            ...
            val winnerStatsMsg = Message.create("BiddingProcessor", "count")
            numberOfMessagesSocket.getOutputStream().write(winnerStatsMsg.serialize())
